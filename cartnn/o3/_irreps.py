...
class Irrep(tuple):
    ...
    @property
    def dim(self) -> int:
        """The dimension of the cartesian representation, :math:`3 ** self.l`."""
        return int(3 ** self.l)
    ...
    def D(self) -> torch.Tensor:
        _, DS, _, _ = ICTD(self.l, self.l, decomposition=True)
        return DS[0]
    
    @property
    def C(self) -> torch.Tensor:
        _, _, CS, _ = ICTD(self.l, self.l, decomposition=False)
        return CS[0]

    @property
    def S(self) -> torch.Tensor:
        _, _, _, SS = ICTD(self.l, self.l, decomposition=False)
        return SS[0]
...
class _MulIr(tuple):
    ...
    @property
    def sdim(self) -> int:
        return self.mul * self.ir.sdim
    
    @property
    def cdim(self) -> int:
        return self.mul * self.ir.cdim
...
class Irreps(tuple):
    ...
    @staticmethod
    def cartesian_harmonics(lmax: int, p: int = -1) -> "Irreps":
        r"""representation of the cartesian_harmonics
        ...
        >>> Irreps.cartesian_harmonics(3)
        1x0e+1x1o+1x2e+1x3o

        >>> Irreps.cartesian_harmonics(4, p=1)
    ...
    def slices_by_ir(self, cartesian: bool = True):
        s = []
        i = 0
        for _, ir in self:
            if cartesian:
    ...
    @property
    def sdim(self) -> int:
        return sum(mul * ir.sdim for mul, ir in self)
    
    @property
    def cdim(self) -> int:
        return sum(mul * ir.cdim for mul, ir in self)
    ...
    def lmin(self) -> int:
        if len(self) == 0:
            raise ValueError("Cannot get lmin of empty Irreps")
        return min(self.ls)
