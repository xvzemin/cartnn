...
from cartnn.util import prod
from cartnn.util.codegen import CodeGenMixin
...
class Linear(CodeGenMixin, torch.nn.Module):
    ...
    Notes
    -----
        `cartnn.o3.Linear` objects created with different partitionings of the same irreps, such as ``Linear("10x0e", "0e")``
    ...
    Parameters
    ----------
    irreps_in : `cartnn.o3.Irreps`
        representation of the input

    irreps_out : `cartnn.o3.Irreps`
        representation of the output

    internal_weights : bool
        whether the `cartnn.o3.Linear` should store its own weights. Defaults to ``True`` unless ``shared_weights`` is
        explicitly set to ``False``, for consistancy with `cartnn.o3.TensorProduct`.

    shared_weights : bool
        whether the `cartnn.o3.Linear` should be weighted individually for each input in a batch. Defaults to ``True``.
    ...
    ----------
    weight_numel : int
        the size of the weights for this `cartnn.o3.Linear`
    ...
    def __init__(
        ...
        *,
        ...
        opt_defaults = cartnn.get_optimization_defaults()
