...
from cartnn import get_optimization_defaults, set_optimization_defaults
import torch
...
_cartnn_COMPILE_MODE = "__cartnn_compile_mode__"
...
def compile_mode(mode: str):
    ...
    def decorator(obj):
        if not (inspect.isclass(obj) and issubclass(obj, torch.nn.Module)):
            raise TypeError("@cartnn.util.jit.compile_mode can only decorate classes derived from torch.nn.Module")
        setattr(obj, _cartnn_COMPILE_MODE, mode)
        return obj
...
def get_compile_mode(mod: torch.nn.Module) -> str:
    ...
    """
    if hasattr(mod, _cartnn_COMPILE_MODE):
        mode = getattr(mod, _cartnn_COMPILE_MODE)
    else:
        mode = getattr(type(mod), _cartnn_COMPILE_MODE, None)
...
def get_tracing_inputs(
    ...
    Otherwise, attempts to infer the input signature of the module using ``cartnn.util._argtools._get_io_irreps``.
...
def trace_module(mod: torch.nn.Module, inputs: dict = None, check_inputs: list = None, in_place: bool = True):
    ...
    old_mode = getattr(mod, _cartnn_COMPILE_MODE, None)
    ...
    setattr(mod, _cartnn_COMPILE_MODE, "trace")
    ...
    if old_mode is not None:
        setattr(mod, _cartnn_COMPILE_MODE, old_mode)
...
def script(mod: torch.nn.Module, in_place: bool = True):
    ...
    setattr(mod, _cartnn_COMPILE_MODE, "script")
...
@contextmanager
def disable_cartnn_codegen():
    """Context manager that disables the legacy PyTorch code generation used in cartnn."""
